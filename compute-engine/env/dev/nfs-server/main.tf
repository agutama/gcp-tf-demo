terraform {
  ...
  backend "gcs" {
    ...
    prefix = "state/compute-engine/nfs-server/"
  }
}

resource "google_compute_address" "nfs-sea2-internal-ip" {
  count        = 1
  name         = "nfs-sea2-internal-ip"
  project      = "cukzlearn03"
...
module "default" {
  ...
  static_internal_ip_address  = google_compute_address.nfs-sea2-internal-ip[0].self_link

  #LABEL
  env           = "dev"
  service_group = "infra"
  service_name  = "nfs"
  service_type  = "file-server"

  #Network Tags
  additional_tags = ["nfs-server", "ssh"]
}